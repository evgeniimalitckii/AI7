...
model = load_model('C:\\Users\\user\\Documents\\TDS data\\interactive plot for HSP model\\HSPmodel_ff_43.h5')
# Set up data
...
def predict(doc):
    ...
    left = figure(title = 'Termal Desorption Spectra', plot_height=400,# plot_width=600,
           x_axis_label = 'Temperature, K', y_axis_label = 'Hydrogen Desorption Rate, at.ppm / s')
    #p.y_range = Range1d(start=0, end=max(y))
    ...
    inputs = column(peak1_amplitude, peak1_temperature, peak1_width,
                   peak2_amplitude, peak2_temperature, peak2_width,
                   peak3_amplitude, peak3_temperature, peak3_width,
                   peak4_amplitude, peak4_temperature, peak4_width,
                   peak5_amplitude, peak5_temperature, peak5_width)
    ...
    results = column(p)
    doc.add_root(row(inputs, results, sizing_mode = "scale_both"))

server = Server({'/': predict}, num_procs=1)
server.start()
